        ...
        stage("Helm Chart Preparation") {
            steps {
                  sh '''
                    ...
                    new_image_tag=`curl -s -H "Authorization: JWT ${TOKEN}" https://hub.docker.com/v2/repositories/senthilnathanam/nginx-realip/tags/?page_size=100 | jq -r '.results|.[]|.content_type+" "+.name' | grep image | awk 'NR==1{print $1}'`
